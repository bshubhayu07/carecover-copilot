...
function triggerBlobDownload(blobContent, fileName) {
...
export function generatePolicySummaryPDF(policyProfile, topupProfile = null) {
  ...
  const insurerSafe = (primary.insurer_name || 'Niva_Bupa').replace(/[^a-zA-Z0-9]/g, '_');
  const fileName = `CareCover_Policy_Summary_${insurerSafe}.pdf`;
  ...
  try {
    ...
    console.warn('jsPDF save failed. Triggering Blob PDF fallback download.', err);
...
export function generatePreAuthFormPDF(policyProfile) {
  ...
  const insurerSafe = (primary.insurer_name || 'Niva_Bupa').replace(/[^a-zA-Z0-9]/g, '_');
  const fileName = `TPA_PreAuth_Form_${insurerSafe}.pdf`;
  ...
  try {
    ...
    secA.forEach(([label, val]) => {
      ...
      doc.setFontSize(8.5);
      doc.setTextColor(51, 65, 85);
      doc.text(String(label), 18, y + 5.5);

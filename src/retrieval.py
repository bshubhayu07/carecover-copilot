...
def ask_policy_question(query: str, collection=None, policy_profile=None) -> str:
    """
    Queries vector collection for relevant clauses and synthesizes high-speed response.
    Optimized for sub-second intent classification and guardrails.
    """
    ...
    # 1. Greeting & Capability Orientation Intent (Instant Sub-10ms Response)
    ...
    # 2. Standardized Out-of-Domain Query Intent (Instant Sub-10ms Response)
    ...
    # 3. Non-Policy Clinical Staffing Query Intent (Instant Sub-10ms Response)
    ...
    # 4. Fast-Path Health Insurance Intent Classification (Ultra-Fast <50ms Response)
    ...
    # 5. Vector Store & LLM Network Chain (Fallback for complex custom queries)
    if not USE_DUMMY_MODE and collection and OPENAI_API_KEY:
        try:
...
Answer the user's question using ONLY the retrieved policy clauses below.
If the evidence is insufficient, explicitly say that the document does not establish the answer.
End with: “Please confirm final eligibility and authorization with the insurer and hospital.”
...
{query}"""
            ...
            print(f"RAG Chain Exception: {e}")

    # Default instant fallback
